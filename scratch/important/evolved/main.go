...
func UseCorticalBeat() {
	...
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Cycle Printer", PrintCycle, func(imp *std.Impulse) bool {
		return imp.Beat == 0
	})

	cortex.Synapses() <- std.NewSynapse(life.Looping, "Refractory Printer", PrintRefractory, func(imp *std.Impulse) bool {
		return imp.Beat == 1
	})

	cortex.Synapses() <- std.NewSynapse(life.Looping, "Response Time Printer", PrintResponse, func(imp *std.Impulse) bool {
		return imp.Beat == 2
	})
}
...
func ShowImperformantSynapticActivity() {
	...
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Cycle Printer", PrintCycle, when.Frequency(1))
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Refractory Printer", PrintRefractory, when.Frequency(1))
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Response Time Printer", PrintResponse, when.Frequency(1))
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Cycle Printer", PrintCycle, when.Frequency(1))
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Refractory Printer", PrintRefractory, when.Frequency(1))
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Response Time Printer", PrintResponse, when.Frequency(1))
	cortex.Synapses() <- std.NewSynapse(life.Looping, "Cycle Printer", PrintCycle, when.Frequency(1))
