...
func NilAny() bounded.Pattern[any] {
	fn := func(i uint) any {
		return nil
	}
	fns := func(i uint) []any {
	...
	return bounded.NewPattern[any](&cursor, fn, fn, fn, fns, fns, fns, nil)
}

// Nil returns a pattern which always yields nil.
func Nil[T any]() bounded.Pattern[*T] {
	return Zero[*T]()
}

// Zero returns a pattern which always yields the zero value of T.
func Zero[T any]() bounded.Pattern[T] {
	var zero T
	fn := func(i uint) T {
		return zero
	}
	...
	cursor, _ := bounded.By[uint](0, 0, 0)
	return bounded.NewPattern[T](&cursor, fn, fn, fn, fns, fns, fns, zero)
}

// One is a pattern of a numeric one.
func One[T num.Primitive]() bounded.Pattern[T] {
	return From[T](1)
}
...
func ZeroOne[T num.Primitive]() bounded.Pattern[T] {
	return From[T](0, 1)
}
...
//
// NOTE: Patterns are stored as you would read them (left→to→right) but are evaluated in a direction of travel.
func OneZero[T num.Primitive]() bounded.Pattern[T] {
	return From[T](1, 0)
}
...
func From[T any](data ...T) bounded.Pattern[T] {
	if len(data) == 0 {
		var zero T
	...
	return bounded.NewPattern[T](&c, walkEast, walkWest, walkTo, yieldEast, yieldWest, yieldTo, data...)
}
