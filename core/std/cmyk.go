...
func NewCMYK[T num.Primitive]() CMYK[T] {
	return CMYK[T](NewCMYKTyped[T, T, T, T]())
}

func NewCMYKTyped[TC num.Primitive, TM num.Primitive, TY num.Primitive, TK num.Primitive]() CMYKTyped[TC, TM, TY, TK] {
	...
	c := CMYKTyped[TC, TM, TY, TK]{}
	c.Entity = NewEntity[format.Default]()
	c.SetBoundaries(minC, maxC, minM, maxM, minY, maxY, minK, maxK)
	return c
}

func (_v CMYKTyped[TC, TM, TY, TK]) String() string {
	if support.AllSameTypes(TC(0), TM(0), TY(0), TK(0)) {
		return fmt.Sprintf("cmyk[%T].{%v, %v, %v, %v}(%v)", TC(0), _v.C.ValueString(), _v.M.ValueString(), _v.Y.ValueString(), _v.K.ValueString(), _v.GivenName.Name)
	}
	return fmt.Sprintf("cmyk[%T, %T, %T, %T].{%v, %v, %v, %v}(%v)", TC(0), TM(0), TY(0), TK(0), _v.C.ValueString(), _v.M.ValueString(), _v.Y.ValueString(), _v.K.ValueString(), _v.GivenName.Name)
}
