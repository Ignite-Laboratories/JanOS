...
func NewRGBA[T num.Primitive]() RGBA[T] {
	return RGBA[T](NewRGBATyped[T, T, T, T]())
}

func NewRGBATyped[TR num.Primitive, TG num.Primitive, TB num.Primitive, TA num.Primitive]() RGBATyped[TR, TG, TB, TA] {
	...
	c := RGBATyped[TR, TG, TB, TA]{}
	c.Entity = NewEntity[format.Default]()
	c.SetBoundaries(minR, maxR, minG, maxG, minB, maxB, minA, maxA)
	return c
}

func (_v RGBATyped[TR, TG, TB, TA]) String() string {
	if support.AllSameTypes(TR(0), TG(0), TB(0), TA(0)) {
		return fmt.Sprintf("rgba[%T].{%v, %v, %v, %v}(%v)", TR(0), _v.R.ValueString(), _v.G.ValueString(), _v.B.ValueString(), _v.A.ValueString(), _v.GivenName.Name)
	}
	return fmt.Sprintf("rgba[%T, %T, %T, %T].{%v, %v, %v, %v}(%v)", TR(0), TG(0), TB(0), TA(0), _v.R.ValueString(), _v.G.ValueString(), _v.B.ValueString(), _v.A.ValueString(), _v.GivenName.Name)
}
