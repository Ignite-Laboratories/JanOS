...
// Semantic represents any recorded comparable data type, including std.Data, and the standard semantics of how to
// express data from these sets.
//
// NOTE: 'context' is a simplified alias for Context.
type Semantic[T any] interface {
	*std.Data[T] | *Unique[T] | *context[T] | *Experience[T]

	// SetYield should overwrite the current 'yield' function with the one provided.
	SetYield(func() T)
	...
	// Yield should emit 'n' elements on demand, which the set should simultaneously record.
	Yield(n uint) []T
